...
@implementation InKindDonationArchiver

+ (InKindDonationArchiver *)sharedInstance {
  static InKindDonationArchiver *s_MySingleton = nil;
  
  @synchronized([InKindDonationArchiver class]) {
...
- (id)initSingleton {
  if (self = [super initSingleton]) {
    [self setPathToLatexBuild:@"/Library/Application Support/BicycleKitchenPOS/ArchivedInKindDonations/latex-build"];
    [self setPathToDonationArchive:@"/Library/Application Support/BicycleKitchenPOS/ArchivedInKindDonations"];
  }
  return self;
}
...
- (NSData *)latexTemplateForDonation:(InKindDonation *)don {
  ...
  [self addString:@"\\input kitchenInKindDonation\n" toData:tmp];
  ...
  unsigned int i, count = [[don items] count];
  for (i = 0; i < count; i++) {
    InKindDonationItem *obj = (InKindDonationItem *)[[don items] objectAtIndex:i];
    [self addString:[NSString stringWithFormat:@"\\itemrow{%@}\n", [obj item]] toData:tmp];
  }
...
- (NSString *)filenameForInKindDonation:(InKindDonation *)don withExtension:(NSString *)ext {
  return [NSString stringWithFormat:@"in-kind-donation-%@.%@", [don uid], ext];
}

- (NSString *)pathToTexBuildFile {
  return [[NSString stringWithFormat:@"%@/inKindDonation.tex", [self pathToLatexBuild]]
...
- (NSString *)pathToOldPdfFile {
  return [[NSString stringWithFormat:@"%@/inKindDonation.pdf", [self pathToLatexBuild]]
    stringByStandardizingPath];
}

- (NSString *)pathToNewPdfFile:(InKindDonation *)don {
  return [[NSString stringWithFormat:@"%@/%@",
    [self pathToTodaysArchive], [self filenameForInKindDonation:don withExtension:@"pdf"]]
    stringByStandardizingPath];
}
